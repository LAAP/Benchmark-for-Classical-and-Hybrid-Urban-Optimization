...
def snapshot_for_cap(cap: int) -> Dict[str, Any]:
    w = ObjectiveWeights(
        active_sectors=1.0,
        skyline_height=1.0,
        compactness=1.0,
...
def main() -> None:
    caps = [24, 32, 48]
    out_dir = Path(__file__).resolve().parents[3] / "evidence" / "small_cap_increase_audit_seed123"
    out_dir.mkdir(parents=True, exist_ok=True)
    snaps = {str(c): snapshot_for_cap(c) for c in caps}
    payload = {"preset": "small", "seed": 123, "sector_balanced_candidates": True, "caps": caps, "snapshots": snaps}
